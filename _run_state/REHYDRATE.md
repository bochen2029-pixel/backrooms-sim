...
```
git -C C:\backrooms log --oneline -12          # HEAD should be 0e0a783 (apparition 2b.2) or later; newest tag phase2b-rt
git -C C:\backrooms status --short             # clean except ?? _brainstorm/ + ?? _staged_rt_perf_{ghost,A,B}/ + ?? _staged_phase2b_backup/ (all applied+committed; removable)
...
**State:** all green @ HEAD `0e0a783` (newest tag `phase2b-rt`; apparition arc complete), pushed. ctest **110/110**, determinism record==replay,
...
**THE OPEN NEXT PICKS (operator's call).** The apparition arc is now COMPLETE: Phase 1 (ADR-082) · 2a player-POV +
audio (ADR-083) · `app_strength` (ADR-083a) · **2b the visual dread dim, raster + RT (ADR-084, E26/E27, tags
`phase2b-raster` `c6ac06f` / `phase2b-rt` `0e0a783`)** — when you see a face the PA murmurs, the room thins, and the
lights sag, on both render paths, all gate-green. Strong candidate now: **PLAYTEST** (RT is fast AND the atmosphere
lands — see the feel; tunables `maxDim`/`apparitionWindowS` in `main.cpp`, two sites raster+RT, keep in sync). The
standing engineering thread is **Phase C.2 (KeelBroker arbitration)** — now FIVE multimodal/
